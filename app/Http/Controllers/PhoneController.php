...
class PhoneController extends Controller
{
    ...
    {
        // Normalizar entrada
        $clean = ChilePhoneHelper::normalize($number);

        if (!$clean) {
            return redirect()->route('home')->with('error', 'El número ingresado no corresponde a un formato válido de teléfono chileno (9 dígitos).');
        }
        ...
        if (!$phone) {
            $areaInfo = ChilePhoneHelper::getAreaInfo($clean);
            $phone = Phone::create([
                'number' => $clean,
        ...
        // Números relacionados de la misma área
    ...
    public function storeComment(Request $request, string $number): RedirectResponse
    {
        ...
        if (!empty($request->input('website_hp'))) {
            return back()->with('success', '¡Gracias! Tu reporte fue registrado y ayudará a otros usuarios en Chile a prevenir llamadas molestas o estafas.');
        }
    ...
    public function voteReason(Request $request, string $number): RedirectResponse
    {
        ...
        // Notificar al Topic de Chile en Telegram (denuncia rápida)
        try {
            \App\Services\TelegramAlertService::sendSpamReport($phone, $comment, 'CL', isFastVote: true, source: 'Web');
        } catch (\Throwable $e) {}
    ...
    {
        $clean = ChilePhoneHelper::normalize($number);
        $formatted = ChilePhoneHelper::format($clean ?? $number);

        $vcard = "BEGIN:VCARD\r\n";
        $vcard .= "VERSION:3.0\r\n";
        $vcard .= "FN:SPAM {$formatted} (Bloquear)\r\n";
        $vcard .= "N:;SPAM {$formatted} (Bloquear);;;\r\n";
        $vcard .= "TEL;TYPE=CELL,VOICE:+56{$clean}\r\n";
        $vcard .= "NOTE:Reportado como llamada molesta / spam en cl.quienllama.com.es\r\n";
        $vcard .= "END:VCARD\r\n";
